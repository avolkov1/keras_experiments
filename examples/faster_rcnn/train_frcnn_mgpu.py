...
# pass the settings from the command line, and persist them in the config
# object
C = config.Config()
...
for epoch_num in range(num_epochs):
    ...
    noutputs = len(model_rpn.outputs)
    while True:
        try:
            ...
            while True:
                ...
                if X.shape in dgroups:
                    cat_flag = True
                else:
                    cat_flag = False

                ref = dgroups.setdefault(X.shape, {'x': X, 'y': Y, 'cnt': 1})

                print('X SHAPE: {}'.format(ref['x'].shape))

                for jj in range(noutputs):
                    print('Y {} SHAPE: {}'.format(jj, ref['y'][jj].shape))

                print('IMG_DATA: {}'.format(img_data))

                if cat_flag:
                    ref['x'] = np.concatenate([ref['x'], X])
                    ref['y'] = [np.concatenate(yy) for yy in zip(ref['y'], Y)]
                    ref['cnt'] = ref['cnt'] + 1
                    if ref['cnt'] == nbatch_lim:
                        data_to_run = ref
                        break
